...
//    24 85	9,26
//    private static Double[] TempTransfer = new Double[26];
//	private static double[] TemperatureRaw = {23.0, 30.0, 24.0, 22.0, 20.0, 23.0, 30.0, 30.0, 23.0, 30.0 };
	private static Double[] TemperatureRaw = new Double[26];
	private static Double[] HumidityRaw = new Double[26];
	private static Double[] WindSpeedRaw = new Double[26];
	private static String[] FinalWeatherRaw = new String[26];
//	private static double[] HumidityRaw =    {85.0, 60.0, 85.0, 85.0 ,95.0, 95.0, 65.0, 65.0, 85.0, 60.0 };
//    private static double[] WindSpeedRaw =   {3.704, 27.78, 9.26, 9.26, 9.26, 3.704, 27.78, 9.26, 3.704, 3.704 };
//    private static String[] Class ={"cerah","cerah","cerah","cerah","cerah","cerah berawan","cerah berawan","berawan","berawan","hujan ringan"};
    
//    double arWind[] = new double[(int) finalDataWind()];
    private static Double[] Temperature = arrDataTempA();
	private static Double[] Humidity = arrDataHumA();
	private static Double[] WindSpeed = arrDataWindA();
	private static String[] FinalWeather = arrDataWindA();
	
	
	public static Double[] arrDataTempA() {
		try {
			readRest.setExcelFile();
			readRest.setArray("Temperatur");
			for (int i = 0; i <= 9; i++) {
				TemperatureRaw[i] = readRest.getCellData("Temperatur",i+1);
			}
			for (int i = 0; i <= 9; i++) {
				if (TemperatureRaw[i] <= 25) 
					TemperatureRaw[i] = (double) 0;
				
				else if (TemperatureRaw[i] > 25) 
					TemperatureRaw[i] = (double) 1;
			}
		} catch (Exception e) {
	...
	public static Double[] arrDataHumA() {
		try {
			readRest.setExcelFile();
			readRest.setArray("Kelembapan");
			for (int i = 0; i <= 9; i++) {
				HumidityRaw[i] = readRest.getCellData("Kelembapan",i+1);
			}
			for (int i = 0; i <= 9; i++) {
				if (HumidityRaw[i] <= 25) 
					HumidityRaw[i] = (double) 0;
				
				else if (HumidityRaw[i] > 25) 
					HumidityRaw[i] = (double) 1;
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static Double[] arrDataWindA() {
		try {
			readRest.setExcelFile();
			readRest.setArray("Kecepatan Angin");
			for (int i = 0; i <= 9; i++) {
				WindSpeedRaw[i] = readRest.getCellData("Kecepatan Angin",i+1);
			}
			for (int i = 0; i <= 9; i++) {
				if (WindSpeedRaw[i] <= 25) 
					WindSpeedRaw[i] = (double) 0;
				
				else if (WindSpeedRaw[i] > 25) 
					WindSpeedRaw[i] = (double) 1;
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public static Double[] FinalWeather() {
		try {
			readRest.setExcelFile();
			readRest.setArray("Cuaca");
			for (int i = 0; i <= 9; i++) {
				FinalWeatherRaw[i] = readRest.getCellData("Cuaca",i+1);
			}
			for (int i = 0; i <= 9; i++) {
				if (FinalWeatherRaw[i] <= 25) 
					FinalWeatherRaw[i] = (double) 0;
				
				else if (FinalWeatherRaw[i] > 25) 
					FinalWeatherRaw[i] = (double) 1;
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
